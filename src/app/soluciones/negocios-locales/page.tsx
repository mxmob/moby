...
export default function NegociosLocalesPage() {
  ...
  return (
    <main>
      <PageHero
        h1={t("soluciones.sectors.negociosLocales.hero.h1")}
        subtitle={t("soluciones.sectors.negociosLocales.hero.subtitle")}
      ...
      <Section>
        <ProblemSolution
          ...
          problemText={t("soluciones.sectors.negociosLocales.problem")}
          solutionTitle="Nuestra solución"
          solutionText={t("soluciones.sectors.negociosLocales.solution")}
        />
      </Section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-center gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <span className="text-white/80 text-sm">{t(`soluciones.sectors.negociosLocales.services.${i}`)}</span>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section>
        <div className="max-w-2xl mx-auto text-center">
          <span className="inline-block px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-xs font-medium mb-4">
            {t("soluciones.sectors.negociosLocales.caseStudy.title")}
          </span>
          <p className="text-white/70 text-lg italic">
            &ldquo;{t("soluciones.sectors.negociosLocales.caseStudy.text")}&rdquo;
          </p>
        </div>
      </Section>

      <CTASection title="¿Tienes un negocio local?" buttonText={t("cta.contact")} />
    </main>
  );
}
