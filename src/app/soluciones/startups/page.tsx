...
export default function StartupsPage() {
  ...
  return (
    <main>
      <PageHero
        h1={t("soluciones.sectors.startups.hero.h1")}
        subtitle={t("soluciones.sectors.startups.hero.subtitle")}
        badge="🚀 Startups"
      />
      ...
      <Section>
        <ProblemSolution
          ...
          problemText={t("soluciones.sectors.startups.problem")}
          solutionTitle="Nuestra solución"
          solutionText={t("soluciones.sectors.startups.solution")}
        />
      </Section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-center gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <span className="text-white/80 text-sm">{t(`soluciones.sectors.startups.services.${i}`)}</span>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section>
        <div className="max-w-2xl mx-auto text-center">
          <span className="inline-block px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-xs font-medium mb-4">
            {t("soluciones.sectors.startups.caseStudy.title")}
          </span>
          <p className="text-white/70 text-lg italic">
            &ldquo;{t("soluciones.sectors.startups.caseStudy.text")}&rdquo;
          </p>
        </div>
      </Section>

      <CTASection title="¿Tienes una startup?" buttonText={t("cta.contact")} />
    </main>
  );
}
